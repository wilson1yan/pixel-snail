...
class PixelSNAIL(nn.Module):
    ...
    def __init__(self, obs_dim, dropout_p=0.5, nr_resnet=5,
        ...
        for attn_rep in range(attn_rep):
            ...
            attn_block.append(gated_resnet(nr_filters, nin, skip_connection_filters=nr_filters)) # rn+5
    ...
    def _create_background(self, obs_dim):
        _, h, w = obs_dim
        base = np.zeros((1, 1, h, w)).astype(np.float32)
    ...
    def forward(self, x):
        ...
        for attn_rep in range(self.attn_rep):
            ...
            rn = self.nr_resnet
            ul = ul_list[-1]
            raw_content = torch.cat((x, ul, self.background.repeat(xs[0], 1, 1, 1)), dim=1)
            raw = attn_block[rn+1](attn_block[rn](raw_content))
            key, mixin = raw[:, :q_size, :, :], raw[:, q_size:, :, :]
            raw_q = torch.cat((ul, self.background.repeat(xs[0], 1, 1, 1)), dim=1)
